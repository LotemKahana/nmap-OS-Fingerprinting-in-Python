...
def ecn_test(probes):
    ...
    test["T"] = utils.t_test(ecn_answer.ttl, probes["u"])
    test["TG"] = utils.tg_test(ecn_answer.ttl)
    test["W"] = utils.w_test(ecn_answer)
    test["O"] = utils.generate_option_string(ecn_answer.payload.options)
...
def t_1_test(probes):
    ...
    test["T"] = utils.t_test(t_answer.ttl, probes["u"])
...
def u1_test(probes):
    ...
    test["T"] = utils.t_test(u_answer.ttl, probes["u"])
...
def ie_test(probes):
    ...
    test["T"] = utils.t_test(ie_answer_first.ttl, probes["u"])
